...
def parse_args():
    ...
    compat_group.add_argument(
        '--counts-dir',
        help='The input directory with read isoform compatibility files')
    ...
    has_counts = args.counts_dir is not None
...
def run_stat_model_with_out_handles(counts_dir, abundance, covars,
    ...
    try:
        run_stat_model_main_thread(counts_dir, abundance, all_samples,
...
def process_counts_tsv(counts_tsv, all_samples, status_counts, on_asm_handler,
    ...
    # counts_tsv is sorted by asm_i and then read_i
    expected_headers = [
...
def run_stat_model_main_thread(counts_dir, abundance, all_samples,
    ...
    if abundance is not None:
        ...
        process_counts_dir(counts_dir, all_samples, status_counts,
...
def run_stat_model(counts_dir, abundance, covar_tsv, sample_read_total_tsv,
    ...
    try:
        ...
        run_stat_model_with_out_handles(
            counts_dir, abundance, covars, min_asm_reads_by_sample, group_1,
    ...
    calculate_adjusted_p_values(coeff_path, lrtp_path, differential_asms_path,
                                differential_isoforms_path)
    has_asm_ids = counts_dir is not None
...
def main():
    ...
    run_stat_model(args.counts_dir, args.abundance, args.covar_tsv,
