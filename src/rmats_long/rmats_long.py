...
def parse_args():
    ...
    required_group.add_argument(
        ...
        help=('The path to a file listing the sample names for group 1. The'
              ...
              ' ESPRESSO abundance column names or --asm-counts-dir names.'))
    required_group.add_argument(
        '--group-2',
        required=True,
    ...
    asm_args = [
        args.event_dir, args.asm_counts_dir, args.align_dir, args.gtf_dir
    ]
...
def count_significant_isoforms(out_dir, diff_transcripts, adj_pvalue,
    ...
    rmats_long_exe = rmats_long_utils.get_rmats_long_executable()
    count_script = 'count_significant_isoforms.py'
    if is_asm:
    ...
    command = [
        rmats_long_exe, count_script, '--diff-transcripts', diff_transcripts,
        '--diff-asms', diff_asms, '--out-tsv', out_tsv, '--adj-pvalue',
...
def detect_differential_isoforms(
        abundance, asm_counts_dir, out_dir, group_1, group_2, adj_pvalue,
    ...
    rmats_long_exe = rmats_long_utils.get_rmats_long_executable()
    detect_script = 'detect_differential_isoforms.py'
    command = [
        rmats_long_exe, detect_script, '--out-dir', out_dir, '--group-1',
    ...
    if abundance:
        ...
        command.extend(['--asm-counts-dir', asm_counts_dir])
...
def visualize_asm_in_gene(event_dir, chr_id, asm_id_str, gencode_gtf, asm_gtf,
    ...
    with tempfile.TemporaryDirectory(suffix='_tmp',
        ...
        rmats_long_exe = rmats_long_utils.get_rmats_long_executable()
        visualize_script = 'visualize_isoforms.py'
        command = [
            rmats_long_exe, visualize_script, '--gene-id', gene_id,
...
def visualize_isoforms(event_dir, chr_id, gene_id, abundance, updated_gtf,
    ...
    visualize_script = 'visualize_isoforms.py'
    command = [
        rmats_long_exe, visualize_script, '--gene-id', gene_id, '--abundance',
        abundance, '--updated-gtf', updated_gtf, '--diff-transcripts',
        diff_transcripts, '--out-dir', out_dir, '--group-1', group_1,
        '--group-2', group_2, '--group-1-name', group_1_name, '--group-2-name',
        group_2_name, '--plot-file-type', plot_file_type, '--intron-scaling',
        str(intron_scaling), '--max-transcripts',
        str(max_transcripts)
    ]
    if gencode_gtf:
    ...
    if colors_path:
        command.extend(['--out-transcript-colors', colors_path])
...
def classify_isoforms(gene, out_dir, updated_gtf, gencode_gtf,
    ...
    rmats_long_exe = rmats_long_utils.get_rmats_long_executable()
    classify_script = 'classify_isoform_differences.py'
    if second_transcript_id is None:
        ...
        command = [
            rmats_long_exe, classify_script, '--updated-gtf', updated_gtf,
...
def visualize_summary(summary_path, summary_dir):
    rmats_long_exe = rmats_long_utils.get_rmats_long_executable()
    script_name = 'visualize_summary.R'
    command = [rmats_long_exe, script_name, summary_path, summary_dir]
    rmats_long_utils.run_command(command)
...
def load_diff_asm_isoforms_by_chr(path):
    ...
    with open(path, 'rt') as handle:
        ...
        for row in rows:
            parsed_event = rmats_long_utils.parse_asm_id(row['asm_id'])
...
def write_gtf_for_asm_id(asm_id, chr_name, temp_dir, event_reader,
    ...
    with tempfile.NamedTemporaryFile('wt',
        ...
        while True:
            ...
            row_id = row['asm_id']
            parsed_event = rmats_long_utils.parse_asm_id(row_id)
...
def write_diff_transcripts_for_asm_id(asm_id, temp_dir, diff_info):
    ...
    with tempfile.NamedTemporaryFile('wt',
        ...
        for info in diff_info:
            parsed_asm_id = rmats_long_utils.parse_asm_id(info['asm_id'])
...
def plot_splice_graph(event_dir, chr_name, gene_id, asm_id, out_path):
    rmats_long_exe = rmats_long_utils.get_rmats_long_executable()
    plot_script = 'plot_splice_graph.py'
    command = [
        rmats_long_exe, plot_script, '--event-dir', event_dir, '--chr',
...
def plot_simple_splice_graph(updated_gtf, intron_scaling, colors_path,
                             out_path):
    rmats_long_exe = rmats_long_utils.get_rmats_long_executable()
    plot_script = 'plot_simple_splice_graph.py'
    command = [
        rmats_long_exe, plot_script, '--out-file', out_path, '--gtf',
        updated_gtf, '--intron-scaling',
        str(intron_scaling), '--transcript-colors', colors_path,
        '--color-by-isoform', '--ids-from-color-file',
...
def process_asm_event_thread(asm_id, asm_id_str, asm_strand, asm_start,
    ...
    for file_type in plot_file_types:
        graph_out_name = '{}_graph{}'.format(asm_id_str, file_type)
        simple_graph_out_name = '{}_simple_graph{}'.format(
            asm_id_str, file_type)
        graph_out = os.path.join(gene_out_dir, graph_out_name)
        simple_graph_out = os.path.join(gene_out_dir, simple_graph_out_name)
        plot_simple_splice_graph(updated_gtf, intron_scaling, temp_colors_path,
                                 simple_graph_out)
        if not no_splice_graph_plot:
            plot_splice_graph(event_dir, chr_name, gene_id, asm_id_str,
                              graph_out)

    os.remove(temp_colors_path)
    os.remove(abundance)
...
def process_top_asm_events_with_event_reader(
    ...
    with open(count_path, 'rt') as counts_handle:
        for row in rmats_long_utils.row_iterator_for_tsv_with_header(
                counts_handle):
            parsed_asm_id = rmats_long_utils.parse_asm_id(row['asm_id'])
...
def append_is_strict_column_with_handles(in_handle, out_handle, event_reader):
    ...
    for line_i, line in enumerate(in_handle):
        ...
        asm_id_details = rmats_long_utils.parse_asm_id(asm_id)
...
def rmats_long(args):
    ...
    if args.diff_transcripts:
        ...
        detect_differential_isoforms(
            args.abundance, args.asm_counts_dir, args.out_dir, args.group_1,
