...
def parse_args():
    ...
    has_asm = args.asm_counts_dir is not None
...
def detect_isoforms(abundance_path, asm_counts_dir, is_asm, use_drimseq,
                    out_dir, num_threads, group_1_path, group_2_path,
                    covar_tsv, min_isoform_reads, min_cpm_per_asm,
                    limit_asm_to_top_n_isoforms, sample_read_total_tsv):
    rmats_long_exe = rmats_long_utils.get_rmats_long_executable()
    if use_drimseq:
        script_name = 'detect_differential_isoforms.R'
        command = [
            rmats_long_exe, script_name, abundance_path, out_dir,
            str(num_threads), group_1_path, group_2_path
        ]
        if covar_tsv:
            command.append(covar_tsv)
    else:
        script_name = 'run_stat_model.py'
        command = [
            rmats_long_exe, script_name, '--out-dir', out_dir, '--num-threads',
        ...
        if is_asm:
            command.extend(['--counts-dir', asm_counts_dir])
        else:
...
def calculate_isoform_proportion(out_dir, abundance_path, is_asm, group_1_path,
                                 group_2_path, sample_read_total_tsv):
    rmats_long_exe = rmats_long_utils.get_rmats_long_executable()
    proportion_script_name = 'calculate_isoform_proportion.py'
    if is_asm:
    ...
    command = [
        rmats_long_exe, proportion_script_name, '--diff-transcripts',
...
def count_significant_isoforms(out_dir, diff_transcripts, diff_asms,
                               adj_pvalue, use_unadjusted_pvalue,
                               delta_proportion, average_reads_per_group,
                               average_cpm_per_group, min_cpm_per_group,
                               asm_proportion_of_gene, group_1_path,
                               group_2_path, is_asm):
    rmats_long_exe = rmats_long_utils.get_rmats_long_executable()
    count_script_name = 'count_significant_isoforms.py'
    if is_asm:
    ...
    command = [
        rmats_long_exe, count_script_name, '--diff-transcripts',
...
def detect_differential_isoforms(args):
    is_asm = args.asm_counts_dir is not None
    detect_isoforms(args.abundance, args.asm_counts_dir, is_asm,
    ...
    diff_paths = calculate_isoform_proportion(args.out_dir, args.abundance,
                                              is_asm, args.group_1,
                                              args.group_2,
                                              args.sample_read_total_tsv)
    ...
    count_significant_isoforms(
        args.out_dir, diff_transcripts_path, diff_asms_path, args.adj_pvalue,
