...
def get_transcript_colors(sorted_transcripts, main_transcript_ids,
    ...
    max_non_main = max(0, max_transcripts - num_main)
    if num_non_main > max_non_main:
        non_main_colors = other_transcript_colors(max_non_main)
        non_main_colors.append(collapsed_transcripts_color())
    else:
        non_main_colors = other_transcript_colors(num_non_main)

    colors.extend(non_main_colors)
    return colors
...
def parse_args():
    ...
    parser.add_argument(
        '--out-transcript-colors',
        help='Where to write a .tsv of colors used for transcripts')
...
def plot_abundance(out_paths, cpm_and_proportion_path, colors,
    ...
    r_script_name = 'visualize_isoforms.R'
    transcript_colors_string = ','.join(colors)
    group_colors_string = ','.join(group_colors())
    command = [
        rmats_long_exe, r_script_name, cpm_and_proportion_path,
...
def plot_transcripts(ax, transcripts_to_plot, plot_coords_by_transcript,
    ...
    for transcript_i, transcript_id in enumerate(reversed_transcripts):
        ...
        used_transcript_colors['transcripts'].append(transcript_id)
        used_transcript_colors['colors'].append(color)
    ...
    used_transcript_colors['transcripts'].reverse()
    used_transcript_colors['colors'].reverse()
...
def plot_structure(out_paths, sj_count_out_paths, parsed_gencode_details,
    ...
    return plot_structure_with_transcripts_and_colors(
...
def plot_structure_with_transcripts_and_colors(
    ...
    used_transcript_colors = {'transcripts': list(), 'colors': list()}
    ...
    sj_count_details = plot_transcripts(
        ...
        font_size, used_transcript_colors)
...
def write_used_transcript_colors(used_transcript_colors,
                                 out_transcript_colors):
    if not out_transcript_colors:
        return

    transcripts = used_transcript_colors['transcripts']
    colors = used_transcript_colors['colors']
    with open(out_transcript_colors, 'wt') as handle:
        headers = ['transcript_id', 'color']
        rmats_long_utils.write_tsv_line(handle, headers)
        for transcript_i, transcript in enumerate(transcripts):
            color = colors[transcript_i]
            columns = [transcript, color]
...
def visualize_isoforms(args):
    ...
    if not args.abundance:
        ...
        used_transcript_colors = plot_structure(
            structure_plot_files, sj_count_plot_files, parsed_gencode_details,
            parsed_updated_details, gene_name, sorted_transcripts,
            edge_weight_by_node, colors, args.max_transcripts,
            args.intron_scaling, args.start_coord, args.end_coord)
        return used_transcript_colors
    ...
    used_transcript_colors = plot_structure(
        structure_plot_files, sj_count_plot_files, parsed_gencode_details,
        parsed_updated_details, gene_name, sorted_transcripts,
        edge_weight_by_node, colors, args.max_transcripts, args.intron_scaling,
...
def main():
    args = parse_args()
    used_transcript_colors = visualize_isoforms(args)
    write_used_transcript_colors(used_transcript_colors,
                                 args.out_transcript_colors)
