...
def find_edges_forward(splice_graph):
    edges = set()
    to_visit = set()
    visited = set()
...
def get_edge_set_from_graph(splice_graph, sink):
    edges_from_forward = find_edges_forward(splice_graph)
    edges_from_backward = find_edges_backward(sink)
    # Only consider edges that are reachable from both the source and the sink.
    # Any other edge could not be used in a splice graph path for a
    # full-length isoform.
    usable_edges = edges_from_forward.intersection(edges_from_backward)
    return usable_edges


def classify_bubbles(bubbles, closest_downstream_by_node,
                     closest_upstream_by_node, splice_graph, sink, strand,
                     output_full_gene_asm, simplify_gene_isoform_endpoints):
    events = list()
    all_edges = get_edge_set_from_graph(splice_graph, sink)
...
def detect_splicing_events_for_gene(
    ...
    for strand in ['+', '-']:
        ...
        elif output_basic_events:
            bubbles = detect_basic_events(results['graph'], strand)
        else:
    ...
    gene_id_value = gene
    for event in events:
        if output_strict_only and not event['is_strict']:
            continue
